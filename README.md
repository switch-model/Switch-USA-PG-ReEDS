...
If you prefer to use the ReEDS standard loads instead of our load growth
...
python pg_to_switch.py pg/settings in/ --case-id s20x1 --year 2030
...
python pg_to_switch.py pg/settings in/ --case-id resource_adequacy --year 2030
...
python pg_to_switch.py pg/settings in/ --case-id p1 --year 2030
```
...
you specify (`in/`), e.g., in `in/2030/s20x1/`.
...
and `study_modules/prepare_next_stage.py`).
...
sbatch pg_to_switch.slurm pg/settings in/ --myopic

# generate two specific cases (--array argument tells which items from the 
# --case-id list to run)
sbatch --array=1-2 pg_to_switch.slurm pg/settings in/ --case-id p1 --case-id s20x1 --myopic
```
...
```
sbatch --array=1 pg_to_switch.slurm pg/settings in/ --case-id s20x1 --myopic
```
