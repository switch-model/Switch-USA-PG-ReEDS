...
def define_components(mod):
    ...
    mod.HYDRO_TS = Set(
        dimen=1,
        ordered=False,
        initialize=lambda m: unique_list(m.tp_to_hts[tp] for tp in m.TIMEPOINTS),
    ...
    mod.TPS_IN_HTS = Set(
        ...
        ordered=False,
        initialize=lambda m, hts: unique_list(
            t for t in m.TIMEPOINTS if m.tp_to_hts[t] == hts
        ),
    ...
    mod.HYDRO_GENS = Set(
        dimen=1,
        initialize=lambda m: unique_list(g for (g, hts) in m.HYDRO_GEN_TS_RAW),
    ...
    mod.HYDRO_GEN_TS = Set(
        ...
        initialize=lambda m: unique_list(
            (g, m.tp_to_hts[tp]) for (g, tp) in m.HYDRO_GEN_TPS
        ),
    )
