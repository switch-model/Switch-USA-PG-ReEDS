...
def main():
    ...
    build_scens_file = jobs["ra_add_difficult_timeseries"]["ecsPropertiesOverride"][
        "taskProperties"
    ][0]["containers"][0]["command"][0]
    with open(build_scens_file, "r") as f:
        # any scenarios to run?
        done = not f.read().strip()

    if done:
        print(f"Ending iteration; no more scenarios to run in {build_scens_file}.")
        return
    ...
    jobs = ["solve_ce", "solve_ra", "ra_add_difficult_days", "ra_iterate_aws"]
    ...
    for job, spec in jobs.items():
        ...
        print(f"Submitted job `{job}`: {job_id}{dep}")
        prev_id = job_id
