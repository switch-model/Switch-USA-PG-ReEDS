...
def define_dynamic_components(m):
    ...
    # constrain SystemCost to be very close to original value (allowing 1e-6
    # slack gives close fit and better solution time than 0 or 1e-5)
    m.system_cost_target = Param(default=0.0, mutable=True)
    m.Cap_System_Cost = Constraint(
        rule=lambda m: m.SystemCost <= m.system_cost_target * 1.000001
    )
    # don't enforce system cost cap yet
    m.Cap_System_Cost.deactivate()
...
def pre_smooth_solve(m):
    ...
    # cap the cost at the current level, with just a little slack
    # TODO: maybe fix construction variables to get a faster solution?
    # TODO: maybe just include a cost of $0.1/MW or so for ramping
    # directly in the main model
    m.system_cost_target = value(m.SystemCost)
    m.Cap_System_Cost.activate()

    m.logger.info("Smoothing free variables.")
